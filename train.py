...
def main():
    ...
    if torch.cuda.device_count() > 1:
        ...
        disp_net = torch.nn.DataParallel(models.DispResNet(args.resnet_layers, args.with_pretrain)).to(device)
        pose_net = torch.nn.DataParallel(models.PoseResNet(18, args.with_pretrain)).to(device)
    else:
